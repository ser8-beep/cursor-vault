...
type UseChoreographyProgressOptions = {
  ...
  /** Global choreography value when internal scroll completes (act 1 end). */
  handoffEnd?: number;
};
...
export function useChoreographyProgress({
  ...
  useEffect(() => {
    handoffEndRef.current = handoffEnd;
  }, [handoffEnd]);

  const applyDocumentProgress = (doc: number) => {
    const end = handoffEndRef.current;
    ...
    if (phase === "idle" && internal <= PROGRESS_EPSILON) {
      effectiveProgress.set(mapRange(clamp01(doc), 0, 1, 0, end));
    }
  };
...
export function mergeScrollProgress(
  ...
  if (phase === "active" || (internalProgress > 0 && internalProgress < 1)) {
    return mapRange(clamp01(internalProgress), 0, 1, 0, handoffEnd);
  }

  if (phase === "completed") {
    if (documentProgress <= handoffDocumentProgress) return handoffEnd;
    return mapRange(documentProgress, handoffDocumentProgress, 1, handoffEnd, 1);
  }

  return mapRange(clamp01(documentProgress), 0, 1, 0, handoffEnd);
}
